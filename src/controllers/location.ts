...
export const logLocation = async (req: Request, res: Response) => {
  try {
    ...
    const locationNamesInLocation = sqltitleCheck[0].map(
      (item: any) => item.location_title
    );
    if (locationNamesInLocation.includes(req.body.location_title)) {
      return res
        .status(400)
        .send(
          "this user already has a location with the title: " +
    ...
    return res
      .status(200)
      .send("New location created! Location_id:" + locationId);
  } catch (err) {
    console.error(err);
...
export const updateLocation = async (req: Request, res: Response) => {
  ...
  //If location with same name exists throw error
  const locationNames = sqltitleCheck[0].map(
    (item: any) => item.location_title
  );
