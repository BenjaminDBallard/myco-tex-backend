...
export const getProbeTherm = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (historical === 'true' && from) {
      sql = `SELECT users.user_id, controller.controller_id, probe_therm.probe_id, probe_therm.probe_therm_id AS measure_id, probe_therm.probe_therm_measure AS measure, UNIX_TIMESTAMP(probe_therm.probe_therm_created_at) AS measure_created_at
      FROM users
      LEFT JOIN location
...
export const logProbeTherm = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json('controllerId: ' + controller_id + ' Type: therm ' + ' Measure: ' + probe_therm_measure)
  } catch (err) {
    console.error(err)
    return res.status(500).send(err)
  } finally {
