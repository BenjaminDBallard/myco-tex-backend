...
export const getProbeHum = async (req: Request, res: Response) => {
  try {
    ...
    let sql;
    if (historical === "true") {
      sql = `SELECT users.user_id, controller.controller_id, probe_hum.*
      FROM users
      LEFT JOIN location
      ...
      WHERE probe.probe_id = ? ORDER BY probe_hum_created_at DESC;`;
    } else {
      sql = `SELECT users.user_id, controller.controller_id, probe_hum.*
      FROM users
      LEFT JOIN location
      ...
      LEFT JOIN probe_hum
        ON probe.probe_id = probe_hum.probe_id
      WHERE probe.probe_id = ? ORDER BY probe_hum_created_at DESC LIMIT 1;`;
    }
