...
LEFT JOIN probe_co2 co2 ON max_co2.probe_id = co2.probe_id AND probe.probe_id = co2.probe_id
LEFT JOIN probe_hum hum ON max_hum.probe_id = hum.probe_id AND probe.probe_id = hum.probe_id
LEFT JOIN probe_ppm ppm ON max_ppm.probe_id = ppm.probe_id AND probe.probe_id = ppm.probe_id
LEFT JOIN probe_therm therm ON max_therm.probe_id = therm.probe_id AND probe.probe_id = therm.probe_id
WHERE (probe_co2_measure IS NOT NULL OR probe_hum_measure IS NOT NULL OR probe_ppm_measure IS NOT NULL OR probe_therm_measure IS NOT NULL)
AND room.room_id = ?
AND ((probe_c02_created_at BETWEEN FROM_UNIXTIME(?) AND FROM_UNIXTIME(?))
  OR (probe_hum_created_at BETWEEN FROM_UNIXTIME(?) AND FROM_UNIXTIME(?))
  OR (probe_ppm_created_at BETWEEN FROM_UNIXTIME(?) AND FROM_UNIXTIME(?))
  OR (probe_therm_created_at BETWEEN FROM_UNIXTIME(?) AND FROM_UNIXTIME(?)))
